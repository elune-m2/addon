"""ID -> name lookups for animations and attachments."""
...
def animation_name(anim_id):
    """Readable name for an AnimationData.dbc id, or ``Anim_<id>``."""
    return ANIMATION_NAMES.get(anim_id, "Anim_%d" % anim_id)


def attachment_name(att_id):
    """Readable name for an attachment id, or ``Attach_<id>``."""
    return ATTACHMENT_NAMES.get(att_id, "Attach_%d" % att_id)
